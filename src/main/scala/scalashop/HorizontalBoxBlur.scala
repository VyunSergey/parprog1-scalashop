...
object HorizontalBoxBlur extends HorizontalBoxBlurInterface {
  ...
  def parBlur(src: Img, dst: Img, numTasks: Int, radius: Int): Unit = {
    val rows = 0 to src.height
    val groupSize = 1 + ((src.height - 1) / numTasks.toDouble).floor.toInt
    val groups = rows.sliding(groupSize + 1, groupSize).toList
    val strips = groups.map(l => (l.head, l.last))
